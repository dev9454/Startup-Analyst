...
class DeepResearchAgent(BaseAgent):
    def __init__(self): super().__init__(name="deep_research")
    ...
    def verify(self, claims: List[str], local_evidence: List[Dict[str,str]]|None=None):
        ...
        user_prompt = (
            "Verify each claim using the context. For each claim set status in "
            "\"supported|mixed|refuted|unknown\", add a 1–2 sentence rationale, and up to 5 evidences (url, quote).\n"
            f"Return EXACT JSON: {VERIFY_SCHEMA}\nOnly JSON."
        )
